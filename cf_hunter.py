...
def parse_args():
    ...
    ap.add_argument("--max-degree", type=int, default=2,
                    help=f"max degree of a(n) and b(n) (<= {MAX_DEGREE_LIMIT}, default 2)")
    ap.add_argument("--coeff-range", type=int, default=3,
                    help=f"coefficients lie in [-R, R] (R <= {MAX_COEFF_LIMIT}, default 3)")
    ...
    ap.add_argument("--mobius-range", type=int, default=4,
    ...
    ap.add_argument("--out-dir", default="cf_hunter_output", help="output directory")
    ...
    args = ap.parse_args()
    if not 0 <= args.max_degree <= MAX_DEGREE_LIMIT:
        ap.error(f"--max-degree must be in [0, {MAX_DEGREE_LIMIT}]")
    if not 0 <= args.coeff_range <= MAX_COEFF_LIMIT:
        ap.error(f"--coeff-range must be in [0, {MAX_COEFF_LIMIT}]")
    if args.depth < 2:
...
def main():
    args = parse_args()
    polys = all_polys(args.max_degree, args.coeff_range)
    n_polys = len(polys)
    total = n_polys * n_polys

    print(f"Search space: a(n), b(n) of degree <= {args.max_degree}, "
          f"coefficients in [-{args.coeff_range}, {args.coeff_range}]")
    print(f"  {n_polys:,} polynomials each -> {total:,} candidate (a, b) pairs")
    ...
    values = [poly_values(c, args.depth) for c in polys]
    b_ok = [any(v[1:]) and all(v[1:]) for v in values]
