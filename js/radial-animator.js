 ...
 */
export function generalizedCollatz(startN, x_param, y_param, z_param) {
    ...
    if (startN === 1) {
        return {
            startN, sequence: [1], steps: 0, max: 1, stoppingTime: "N/A", type: "Converges to 1", converges_to_1: true
        };
    }
    ...
    if (current === 1n) {
        converges_to_1 = true;
        type = "Normal"; // This has been simplified to align with the test's expectation
...
function initialize() {
    ...
    DOMEElements.xValueInput.value = parseInt(urlParams.get('x'), 10) || 2;
    DOMElements.yValueInput.value = parseInt(urlParams.get('y'), 10) || 3;
